...
class AdministrationApi
{
    ...
    public static function getCurrentFile($em, \App\Entity\User $user)
    {
    ...
    public static function getCurrentFileID($em, \App\Entity\User $user)
    {
        $upRepository = $em->getRepository(UserParameter::class);
    ...
    public static function setCurrentFile($em, \App\Entity\User $user, \App\Entity\File $file)
    {
    ...
    public static function setCurrentFileID($em, \App\Entity\User $user, $fileID)
    {
        // Recherche du dossier en cours
        $userParameter = AdministrationApi::getCurrentFile($em, $user);
        if ($userParameter === null) {
    ...
    public static function setCurrentFileIfNotDefined($em, \App\Entity\User $user, \App\Entity\File $file)
    {
        // Recherche du dossier en cours
        $userParameter = AdministrationApi::getCurrentFile($em, $user);
        if ($userParameter === null) {
    ...
    public static function setFirstFileAsCurrent($em, \App\Entity\User $user)
    {
        // Recherche du dossier en cours
        $userParameter = AdministrationApi::getCurrentFile($em, $user);
    ...
    public static function getNumberLines($em, \App\Entity\User $user, $entityCode)
    {
        $upRepository = $em->getRepository(UserParameter::class);
        ...
        if ($userParameter != null) {
            $numberLines = $userParameter->getIntegerValue();
        } else {
            $numberLines =  constant(Constants::class.'::LIST_DEFAULT_NUMBER_LINES');
        }
        return $numberLines;
    }

    // Met à jour le nombre de lignes affichées dans les listes pour une entité donnée
    public static function setNumberLines($em, \App\Entity\User $user, $entityCode, $numberLines)
    {
        $upRepository = $em->getRepository(UserParameter::class);
        $userParameter = $upRepository->findOneBy(array('user' => $user, 'parameterGroup' => ($entityCode.'.number.lines.columns'), 'parameter' => ($entityCode.'.number.lines')));
        if ($userParameter != null) {
    ...
    public static function getNumberColumns($em, \App\Entity\User $user, $entityCode)
    {
        $upRepository = $em->getRepository(UserParameter::class);
        ...
        if ($userParameter != null) {
            $numberColumns = $userParameter->getIntegerValue();
        } else {
            $numberColumns =  constant(Constants::class.'::LIST_DEFAULT_NUMBER_LINES');
        }
        return $numberColumns;
    }

    // Met à jour le nombre de colonnes affichées dans les listes pour une entité donnée
    public static function setNumberColumns($em, \App\Entity\User $user, $entityCode, $numberColumns)
    {
        $upRepository = $em->getRepository(UserParameter::class);
        $userParameter = $upRepository->findOneBy(array('user' => $user, 'parameterGroup' => ($entityCode.'.number.lines.columns'), 'parameter' => ($entityCode.'.number.columns')));
        if ($userParameter != null) {
    ...
    public static function setFileBookingEmailAdministrator($em, \App\Entity\User $user, \App\Entity\File $file, $fileAdministrator)
    {
    ...
    public static function getFileBookingEmailAdministrator($em, \App\Entity\File $file)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
        $fileParameter = $fpRepository->findOneBy(array('file' => $file, 'parameterGroup' => 'booking.mail', 'parameter' => 'administrator'));
        if ($fileParameter != null) {
            $fileAdministrator = $fileParameter->getBooleanValue();
        } else {
            $fileAdministrator =  constant(Constants::class.'::BOOKING_MAIL_ADMINISTRATOR');
        }
    ...
    public static function setFileBookingEmailUser($em, \App\Entity\User $user, \App\Entity\File $file, $bookingUser)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
    ...
    public static function getFileBookingEmailUser($em, \App\Entity\File $file)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
        $fileParameter = $fpRepository->findOneBy(array('file' => $file, 'parameterGroup' => 'booking.mail', 'parameter' => 'user'));
        if ($fileParameter != null) {
            $bookingUser = $fileParameter->getBooleanValue();
        } else {
            $bookingUser =  constant(Constants::class.'::BOOKING_MAIL_USER');
        }
        return $bookingUser;
    }

    // Met à jour l'indicateur: Restriction de période de réservation avant la date du jour.
    public static function setFileBookingPeriodBefore($em, \App\Entity\User $user, \App\Entity\File $file, $before)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
    ...
    public static function getFileBookingPeriodBefore($em, \App\Entity\File $file)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
        $fileParameter = $fpRepository->findOneBy(array('file' => $file, 'parameterGroup' => 'booking.period', 'parameter' => 'before'));
        if ($fileParameter != null) {
            $before = $fileParameter->getBooleanValue();
        } else {
            $before =  constant(Constants::class.'::BOOKING_PERIOD_BEFORE');
        }
        return $before;
    }

    // Met à jour le type de restriction de période de réservation avant la date du jour.
    public static function setFileBookingPeriodBeforeType($em, \App\Entity\User $user, \App\Entity\File $file, $beforeType)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
    ...
    public static function getFileBookingPeriodBeforeType($em, \App\Entity\File $file)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
        $fileParameter = $fpRepository->findOneBy(array('file' => $file, 'parameterGroup' => 'booking.period', 'parameter' => 'before.type'));
        if ($fileParameter != null) {
            $beforeType = $fileParameter->getStringValue();
        } else {
            $beforeType =  constant(Constants::class.'::BOOKING_PERIOD_BEFORE_TYPE');
        }
        return $beforeType;
    }

    // Met à jour le nombre associé à la restriction de période de réservation avant la date du jour.
    public static function setFileBookingPeriodBeforeNumber($em, \App\Entity\User $user, \App\Entity\File $file, $beforeNumber)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
    ...
    public static function getFileBookingPeriodBeforeNumber($em, \App\Entity\File $file)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
        $fileParameter = $fpRepository->findOneBy(array('file' => $file, 'parameterGroup' => 'booking.period', 'parameter' => 'before.number'));
        if ($fileParameter != null) {
            $beforeNumber = $fileParameter->getIntegerValue();
        } else {
            $beforeNumber =  constant(Constants::class.'::BOOKING_PERIOD_BEFORE_NUMBER');
        }
        return $beforeNumber;
    }

    // Met à jour l'indicateur: Restriction de période de réservation après la date du jour.
    public static function setFileBookingPeriodAfter($em, \App\Entity\User $user, \App\Entity\File $file, $after)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
    ...
    public static function getFileBookingPeriodAfter($em, \App\Entity\File $file)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
        $fileParameter = $fpRepository->findOneBy(array('file' => $file, 'parameterGroup' => 'booking.period', 'parameter' => 'after'));
        if ($fileParameter != null) {
            $after = $fileParameter->getBooleanValue();
        } else {
            $after =  constant(Constants::class.'::BOOKING_PERIOD_AFTER');
        }
        return $after;
    }

    // Met à jour le type de restriction de période de réservation après la date du jour.
    public static function setFileBookingPeriodAfterType($em, \App\Entity\User $user, \App\Entity\File $file, $afterType)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
    ...
    public static function getFileBookingPeriodAfterType($em, \App\Entity\File $file)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
        $fileParameter = $fpRepository->findOneBy(array('file' => $file, 'parameterGroup' => 'booking.period', 'parameter' => 'after.type'));
        if ($fileParameter != null) {
            $afterType = $fileParameter->getStringValue();
        } else {
            $afterType =  constant(Constants::class.'::BOOKING_PERIOD_AFTER_TYPE');
        }
        return $afterType;
    }

    // Met à jour le nombre associé à la restriction de période de réservation après la date du jour.
    public static function setFileBookingPeriodAfterNumber($em, \App\Entity\User $user, \App\Entity\File $file, $afterNumber)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
    ...
    public static function getFileBookingPeriodAfterNumber($em, \App\Entity\File $file)
    {
        $fpRepository = $em->getRepository(FileParameter::class);
        $fileParameter = $fpRepository->findOneBy(array('file' => $file, 'parameterGroup' => 'booking.period', 'parameter' => 'after.number'));
        if ($fileParameter != null) {
            $afterNumber = $fileParameter->getIntegerValue();
        } else {
            $afterNumber =  constant(Constants::class.'::BOOKING_PERIOD_AFTER_NUMBER');
        }
        return $afterNumber;
    }
}
